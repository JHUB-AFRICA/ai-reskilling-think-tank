...
| :--- | :--- | :--- | :--- |
| **1. Kickoff** | Scope alignment, role assignment, timeline setup | Project Charter, Role Matrix | `01_Charter/` |
| **2. Planning** | Functional & non-functional scoping, technical architecture | Requirements Document, Risk Log | `02_Requirements/` |
| **3. Execution** | Core backend NLP engine, FastAPI REST, React frontend | Weekly Progress Log, Decision Log | `03_Progress_Logs/`, `04_Decisions_and_Risks/` |
| **4. Testing** | Unit tests (86 passing), integration tests, UI verification | Test Execution Log, Bug Tracker | `06_Testing/` |
| **5. Delivery** | System handover, production readiness, documentation | Handover Doc, Final Report | `09_Closure/`, `11_Final_Report/` |
...
```mermaid
graph TB
    subgraph Frontend["🖥️ Client Layer — CareerDev (React 19 + Vite)"]
        UI["React 19 Dashboard & Wizard"]
        State["TanStack Query (Server State)"]
        AuthClient["Supabase JS Client"]
    end

    subgraph Backend["🐍 Backend Layer — FastAPI"]
        API["FastAPI REST Endpoints (app_api/main.py)"]
        AuthMid["PyJWT Offline Auth (app_api/auth.py)"]
        NLP["spaCy Skill Extraction Engine (reskilling/nlp.py)"]
        Rec["Gap Analysis Engine (reskilling/recommender.py)"]
        LLM["Google Gemini Client (reskilling/llm_reasoning.py)"]
        LRS["xAPI LRS Logger (reskilling/lrs.py)"]
    end

    subgraph Database["🗄️ Database & Storage Layer"]
        SupaDB[("Supabase PostgreSQL")]
        ONETData[("O*NET Taxonomy CSVs")]
        LRSFile[("xAPI Statements (statements.jsonl)")]
    end

    UI --> State
    UI --> AuthClient
    State -- REST Requests --> API
    API --> AuthMid
    API --> NLP
    API --> Rec
    API --> LLM
...
1. **AI Key Dependency:** If `GEMINI_API_KEY` is not provided in `.env`, the `/me/career-guidance/stream` endpoint gracefully degrades with a 500 configuration notice; standard gap analysis remains 100% functional.
...
rootdir: d:\ai project\New folder\files\reskilling-platform-complete\reskilling-platform
collected 86 items
...
| Test ID | Test Target / Scenario | Expected Result | Pass / Fail |
| :--- | :--- | :--- | :--- |
| **TC-01** | `test_extract_skills_valid_resume` | Returns status 200 with extracted skill list | **PASS** |
| **TC-02** | `test_gap_analysis_exact_match` | Match score equals 100% for identical skills | **PASS** |
| **TC-03** | `test_auth_missing_header` | Returns 401 Unauthorized for unauthenticated requests | **PASS** |
| **TC-04** | `test_auth_expired_jwt` | Returns 401 Token Expired for stale JWTs | **PASS** |
| **TC-05** | `test_admin_route_non_admin` | Returns 403 Forbidden for `job_seeker` role | **PASS** |
| **TC-06** | `test_gemini_streaming_fallback` | Returns graceful error chunk if API key missing | **PASS** |
| **TC-07** | `test_lrs_statement_format` | Validates xAPI actor, verb, and object JSON structure | **PASS** |
| **TC-08** | `test_rate_limit_exceeded` | Returns 429 Too Many Requests upon burst threshold | **PASS** |
...
The documentation structure follows strict governance standard PRJ01:

```
docs/PRJ01_Reskilling_Platform_Docs/
├── PRJ01_Master_Project_Documentation_v1.0_2026-08-13.md  <-- Master File
├── 01_Charter/
│   └── PRJ01_ProjectCharter_v1.0_2026-07-01.md
├── 02_Requirements/
│   └── PRJ01_RequirementsDoc_v1.0_2026-07-05.md
├── 03_Progress_Logs/
│   └── PRJ01_WeeklyProgressLog_v1.0_2026-08-13.md
├── 04_Decisions_and_Risks/
│   └── PRJ01_DecisionLog_v1.0_2026-08-13.md
├── 05_System_Documentation/
│   └── PRJ01_SystemDocumentation_v1.0_2026-08-13.md
├── 06_Testing/
│   └── PRJ01_TestingReport_v1.0_2026-08-13.md
├── 07_Stakeholder_and_Comms/
│   └── PRJ01_StakeholderCommsLog_v1.0_2026-08-13.md
├── 08_Knowledge_and_Research/
│   └── PRJ01_KnowledgeAndResearch_v1.0_2026-08-13.md
├── 09_Closure/
│   └── PRJ01_HandoverAndClosure_v1.0_2026-08-13.md
├── 10_Operations/
│   └── PRJ01_OperationsAndEthics_v1.0_2026-08-13.md
├── 11_Final_Report/
│   └── PRJ01_FinalReport_v1.0_2026-08-13.md
├── 12_Assets/
└── Archive/
```
